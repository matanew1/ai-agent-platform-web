...
describe("useAppSettings", () => {
  ...
  it("keeps browser settings scoped to the signed-in user", async () => {
    vi.mocked(getSettings).mockResolvedValue({ theme: "dark", locale: "en", compact: false, reduceMotion: false, showSources: true, showToolActivity: true, highContrast: false, autoReadResponses: false, sendOnEnter: true, englishVoice: "preferred", hebrewVoice: "preferred", speechInputLocale: "auto" });
  ...
  it("does not overwrite a local edit while the initial request is pending", async () => {
    ...
    act(() => result.current.setSettings({ ...result.current.settings, locale: "he" }));
    await act(async () => resolveRemote({ theme: "light", locale: "en", compact: false, reduceMotion: false, showSources: false, showToolActivity: false, highContrast: false, autoReadResponses: false, sendOnEnter: true, englishVoice: "preferred", hebrewVoice: "preferred", speechInputLocale: "auto" }));
